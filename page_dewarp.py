...
ADAPTIVE_OFFSET = 15     # offset for adaptive threshold
...
def get_contours(name, small, pagemask, masktype):
    ...
    for contour in contours:
        ...
        except ZeroDivisionError: # This can happen if area of contour is zero
            pass
...
def remap_image(name, img, small, page_dims, params, return_array=False):
    ...
    thresh = cv2.adaptiveThreshold(remapped, 255, cv2.ADAPTIVE_THRESH_MEAN_C,
                                   cv2.THRESH_BINARY, ADAPTIVE_WINSZ,
                                   ADAPTIVE_OFFSET)
    if return_array:
        return thresh
